...
#define _bessel_h_
extern complex<double> bessj( int, complex<double> );
extern complex<double> bessy( int, complex<double> );
extern complex<double> bessi( int, complex<double> );
extern complex<double> bessk( int, complex<double> );
#endif
#<
*/
...
complex<double> setdblank_c(complex<double> x) { x = 0.0; return x; }

static complex<double> bessj0(complex<double> x)
/*------------------------------------------------------------*/
...
{
    complex<double> ax, z;
    complex<double> xx, y, ans, ans1, ans2;
...
static complex<double> bessj1(complex<double> x)
/*------------------------------------------------------------*/
/* PURPOSE: Evaluate Bessel function of first kind and order  */
/*          1 at input x                                      */
/*------------------------------------------------------------*/
    ...
    if (abs(ax = abs(x)) < 8.0) {
        ...
        ans2 = 0.04687499995 + y * (-0.2002690873e-3
            + y * (0.8449199096e-5 + y * (-0.88228987e-6
                + y * 0.105787412e-6)));
        ans = sqrt(0.636619772 / ax) * (cos(xx) * ans1 - z * sin(xx) * ans2);
        if (x.real() < 0.0 || x.imag() < 0.0) ans = -ans;
    }
    return ans;
}
...
complex<double> bessj(int n, complex<double> x)
/*------------------------------------------------------------*/
/* PURPOSE: Evaluate Bessel function of first kind and order  */
/*          n at input x                                      */
...
{
    int    j, jsum, m;
    complex<double> ax, bj, bjm, bjp, sum, tox, ans;


    if (n < 0)
    {
        complex<double>   dblank = 0.0;
        setdblank_c(dblank);
        return(dblank);
    }
    ax = abs(x);
    ...
    return  abs(x) < 0.0 && n % 2 == 1 ? -ans : ans;
}




static complex<double> bessy0(complex<double> x)
/*------------------------------------------------------------*/
...
static complex<double> bessy1(complex<double> x)
/*------------------------------------------------------------*/
/* PURPOSE: Evaluate Bessel function of second kind and order */
/*          1 at input x.                                     */
/*------------------------------------------------------------*/
...
complex<double> bessy(int n, complex<double> x)
/*------------------------------------------------------------*/
/* PURPOSE: Evaluate Bessel function of second kind and order */
/*          n for input x. (n >= 0)                           */
